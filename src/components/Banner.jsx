    ...
    return (
        <>
            <Swiper
                ...
                className=" text-white w-full lg:w-11/12"
            >
                {slides.map((slide) => (
                    <SwiperSlide key={slide.id}>
                        <div
                            className="relative h-[70vh] lg:h-[80vh] bg-cover bg-center"
                            style={{ backgroundImage: `url(${slide.image})` }}
                        >
                            <div className="absolute inset-0 bg-black bg-opacity-80 flex  ">

                                <div className='flex flex-col justify-center items-start text-left text-white px-20 md:w-3/4 w-full'>
                                    <h1 className="text-4xl md:text-5xl font-bold mb-4">{slide.title}</h1>
                                    <p className="text-sm md:text-xl">{slide.subtitle}</p>
                                    <Link to={"/all"} className='btn rounded-3xl mt-2 btn-wide bg-[#e5904f84] border-none text-white'>See All Foods</Link>
                                </div>
                                <div>
